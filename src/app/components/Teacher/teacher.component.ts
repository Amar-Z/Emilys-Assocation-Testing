...
Proxy,
Teacher,
Params_Get_Teacher_By_Where,
Params_Delete_Teacher,
...
Get_Teacher_By_Where_Subscription = new Subscription();
searchModel: Params_Get_Teacher_By_Where = new Params_Get_Teacher_By_Where();
data: Teacher[] = [];
...
ngOnDestroy(): void {
this.Get_Teacher_By_Where_Subscription.unsubscribe();
...
this.Get_Teacher_By_Where_Subscription = this.proxy.Get_Teacher_By_Where(this.searchModel).subscribe(result => {
 if (result != null) {
result.forEach((element: any) => {
element.MyURL = this.CmSvc.APIUrl + '/Upload_Image?REL_ENTITY=[TBL_TEACHER]&REL_FIELD=TEACHER_IMAGE&REL_KEY=' + element.TEACHER_ID;
...
if (this.data.filter(e => e.TEACHER_ID === -1).length > 0) {
return;
}
}
const record = new Teacher();
record.TEACHER_ID = -1;
this.data.unshift(record);
}
Edit(current) {
this.proxy.Edit_Teacher(current).subscribe((result) => {
if (result != null) {
...
newEntry.MyURL = this.CmSvc.APIUrl + '/Upload_Image?REL_ENTITY=[TBL_TEACHER]&REL_FIELD=TEACHER_IMAGE&REL_KEY=' + newEntry.TEACHER_ID;
...
if (response) {
const _params_Delete_Teacher = new Params_Delete_Teacher();
_params_Delete_Teacher.TEACHER_ID = entry.TEACHER_ID;
this.proxy.Delete_Teacher(_params_Delete_Teacher).subscribe(data => {
if (data === '') {
